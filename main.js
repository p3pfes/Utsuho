...
var killswitch = true //bot killswitched (only debug users / users during global debug mode can trigger)
const prefix = 'ut!' //bot prefix
...
setInterval(() => { //interval that sets the status of the bot to represent the amount of servers its in and saves the data of every user stored in memory
    dat.savecache()
    client.user.setActivity({ name: `Run th!start! \n \n Touhou in ${client.guilds.cache.size} servers.` })
...
setInterval(() => { //interval
    ...
    .then(results => {
        client.user.setActivity({name:`Pokémon in ${results.reduce((acc, guildCount) => acc + guildCount, 0)} servers!`});
    })
    .catch(console.error);
}, 20000)*/
